...
public class PostDumpFlow {
    ...
    private static final String PROJECT_ID = "around-284203";
    ...
    public static void main(String[] args) {
        ...
        // 从BigTable读取数据
        PCollection<Result> btRows = p.apply(Read.from(CloudBigtableIO.read(config))); // BT格式
        PCollection<TableRow> bqRows = btRows.apply(ParDo.of(new DoFn<Result, TableRow>() { // 转化成BQ格式
            @Override
            // 把BT格式数据转成string，再把string转成BQ格式数据
            public void processElement(ProcessContext c) {
                Result result = c.element();  // 一行数据，之后都是从这行数据读取column
        ...
        // 把BQ格式数据写进BigQuery
        TableSchema schema = new TableSchema().setFields(fields); // 创建schema
